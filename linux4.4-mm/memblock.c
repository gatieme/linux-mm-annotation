...
/**
 * 在type查找与[base, base+size)重叠的区域, 返回idx.
 */
 ...
 */
/**
 * 将给定的物理地址所指定的memory region加入到指定的memblock
...
/**
 * 从指定的memblock中移除指定物理地址所指定的memory region.
 * 如果所指定的区域是存在区域的一部分，则涉及到调整region大小，
 * 或者将一个region拆分成为两个region.
 */
...
/**
 * 使用该函数来释放由memblock_alloc申请到的物理内存，释放的内存
 * 会从memblock.reserved中移除，并加入memblock.memory中
 */
...
/**
 * 使用该函数可以向kernel申请一块可用的物理内存。实际的操作是
 * 在memblock.memory中找到合适的内存，将其从memblock.memory去除，
 * 加入到memblock.reserved中以标记其已经被使用。
 */
