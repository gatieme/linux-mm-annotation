 ...
 */
/**
 * 将页面数据写入到交换区指定位置。shrink_list函数激活交换页的IO传输过程。
 * 它检查页框的PG_dirty标志，然后执行pageout函数。交换高速缓存的writepage
 * 方法为本函数。因此，最终的IO传输过程由本函数完成。
 */
		...
		bio_end_io_t end_write_func)
{
	...
	ret = 0;
	/**
	 * 为交换页生成一个bio，并正确填充所需的参数，分配并初始化一个BIO描述符。
	...
	unlock_page(page);
	/**
	 * 提交写请求。 当IO传输完毕后，执行end_swap_bio_write，
	 * 该函数唤醒等待页PG_writeback标志清0的所有进程。
