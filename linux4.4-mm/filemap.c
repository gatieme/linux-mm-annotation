...
void delete_from_page_cache(struct page *page)
{
	...
	/**
	 * __delete_from_page_cache真正从树中删除节点。
	 */
...
/**
 * 把一个新页的描述符插入到页高速缓存。它接收的参数有：
 * 页描述符的地址page、address_space对象的地址mapping、
 * 表示在地址空间内的页索引的值offset和为基数分配新结点
 * 时所使用的内存分配标志gfp_mask
 */
				      ...
				      void **shadowp)
{
	...
	/**
	 * 调用radix_tree_preload，它禁用内核抢占。并把一些空的
	 * radix_tree_node结构赋给每CPU变量radix_tree_preloads。
	...
	/**
	 * 调用radix_tree_preload_end重新启用内核抢占。
	 */
	radix_tree_preload_end();
	if (unlikely(error))
		goto err_insert;
...
no_cached_page:
		...
		/**
		 * 将页面添加到lru缓存中,插入新页描述符到高速缓存中.
		 * 新页的PG_locked标志被置位.将新页插入LRU链表中.
		 */
...
inline ssize_t generic_write_checks(struct kiocb *iocb, struct iov_iter *from)
{
	...
	/**
	 * 如果不是块设备(即普通文件)
	 * 并且指定了IOCB_APPEND，就将ppos设为文件尾。从而将新数据追加到文件的后面。
	 */
...
generic_file_direct_write(struct kiocb *iocb, struct iov_iter *from, loff_t pos)
{
	...
	/**
	 * 如果nrpages不为空，则调用filemap_fdatawrite和filemap_fdatawait
	 * 函数刷新所有脏页到磁盘。并等待IO操作结束。
	 */
	 ...
	 */
	/**
	 * 调用invalidate_inode_pages2扫描mapping基树中所有页
	 * 并释放它们。同时也清空指向这些页的用户态页表项。
	 */
	if (mapping->nrpages) {
...
int try_to_release_page(struct page *page, gfp_t gfp_mask)
{
	...
	/**
	 * 如果定义了块设备的address_space对象的releasepage方法，
	 * 就调用它。该回调函数通常是没有定义的。
	 */
