...
static inline int page_is_buddy(struct page *page, struct page *buddy,
							unsigned int order)
{
	/* 判断页面是否在内存域中。如果内存域有空洞，需要验证其pfn有效性 */
 ...
 * 内存域某种迁移链表中无法分配页面时，从哪些备用链表中分配页面
 */
...
static void wake_all_kswapds(unsigned int order, const struct alloc_context *ac)
{
	...
	/*
	 * 遍历备用列表中的所有内存域，每次都调用wakeup_kswapd()，唤醒负责
	 * 换出页的kswapd守护进程。空闲内存可以通过缩减内核缓存和页面回收
	 * 获得，即写回或换出很少使用的页，这两种措施都是由该守护进程发起的。
	 */
...
/*
 * 对分配标志进行调整，修改为一些在当前特定情况下更有可能分配成功的标志。
 * 同时，将水印降低到最小值。对实时进程和指定了__GFP_WAIT标志因而不能
 * 睡眠的调用，会设置ALLOC_HARDER。
 */
static inline int
...
retry:
	 ...
	 * 避免递归调用。参见__alloc_pages_direct_reclaim()。
	 */
...
void __free_pages(struct page *page, unsigned int order)
{
	if (put_page_testzero(page)) {
		 ...
		 * 如果释放单页，则不还给伙伴系统，而是至于per-CPU
		 * 缓存中，对很可能出现在CPU高速缓存的页，则放置到
		 * 热页的列表中。出于该目的，内核提供了free_hot_page()辅助函数。
		 */
		if (order == 0)
...
/*
 * 用于将2^order页返回给内存管理子系统。addr是虚拟地址
 */
...
static int find_next_best_node(int node, nodemask_t *used_node_mask)
{
	...
	/*
	 * 如果node指向的结点还没有设置，则返回node。也就是说这个时候
	 * 备用列表的第一项还没初始化，第一项是对应于本地的节点。
	 */
 ...
 */
/*
 * 该函数完成了一些不怎么有趣的标准初始化工作，
 * 其中有一件是实质性的，即所有的页最初都标记为可移动的!
 */
 ...
 */
/*
 * 为struct zone结构的pageset数组的第一个
 * 数组元素分配内存。分配第一个数组元素，换句话说，就是意味着
 * 为第一个系统处理器分配。系统的所有内存域都会考虑进来。
 * 该函数还负责设置冷热分配器的限制。请注意，在SMP系统上对应于其他CPU的
 * pageset数组成员，将会在相应的CPU激活时初始化。
 */
 ...
 */
/*
 * zone结构的lowmem_reserve成员的计算由setup_per_zone_lowmem_reserve()
 * 完成。内核迭代系统的所有结点，对每个结点的各个内存域分别计算预留
 * 内存最小值，具体的算法是将内存域中页帧的总数除以
 * sysctl_lowmem_reserve_ratio[zone]。除数的默认设置对低端内存域是
 * 256，对高端内存域是32.
 */
 ...
 */
/*
 * 初始化每个内存域的水线，计算其最小保留内存大小每次通过proc
 * 文件系统修改某个控制参数时也会调用该函数。计算zone结构水印值。
 */
