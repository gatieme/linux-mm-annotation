 ...
 */
/**
 * 将一个页标记为访问过。只有当PG_referenced标志置位时，
 * 它把页从非活动链表移到活动链表。以下情况会调用本函数:
 *		当按需装入进程的一个匿名页时。
 *		当按需装入内存映射文件的一个页时。
 *		当按需装入IPC共享内存区的一个页时。
 *		当从文件读取数据页时。
 ...
 *
 * 如果页不在LRU链表中，将PG_lru标志置位，得到管理区的lru_lock自谢锁，
 * 调用add_page_to_inactive_list把页插入管理区的非活动链表。
 */
 ...
 */
/**
 * 如果页不在LRU链表中，将PG_lru和PG_active标志置位，得到管理区的lru_lock自旋锁，
 * 调用add_page_to_active_list把页插入管理区的活动链表。
 */
