 ...
 * 如果没有用户态进程(除了当前进程)引用该页，或者超过50%的交换
 * 表项在用，则从交换高速缓存中释放该页。
 */
		 ...
		 unsigned long pages_to_unuse)
{
	...
	while ((i = find_next_to_unuse(si, i, frontswap)) != 0) {
		...
		/**
		 * read_swap_cache_async函数换入页，可能还会分配一个新页。用存放在页槽中
		 * 的数据填充新页框。并把这个页存放在交换高速缓存。
		 */
		...
		if (swap_count(*swap_map)) {
			...
			while (swap_count(*swap_map) && !retval &&
				...
				if (!swap_count(swcount)) /* any usage ? */
					...
					/**
					 * unuse_process扫描进程所有页表项，并用这个新页框的
					 * 物理地址替换页表中每个出现的换出页标识符。
					 */
...
SYSCALL_DEFINE1(swapoff, const char __user *, specialfile)
{
	...
	spin_lock(&swap_lock);
	/**
	 * 扫描交换区描述符链表，比较文件对象地址与活动交换区描述符的swap_file。
	 * 如果不一致，说明传给函数的是一个无效参数，返回错误码。
	 */
